...
#using parser to manually add interface and mac address from command line
def get_argumnets():
    parser = optparse.OptionParses()
    parser.add_option("-i", "--interface", dest="interface", help="Changing MAC Address")
    parser.add_option("-m", "--mac", dest="new_mac", help="New MAC address")
    (options, arguments) = get_arguments()
    if not options.interface: # checking  if interface was given
       parser.error("[-] Please enter an interface, use --help for info")
       get_arguments()
    elif not options.new_mac: # checking if new mac address was given
